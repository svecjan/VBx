...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default='nocat', help='Task - ("nocat", "cat")')
    ...
    parser.add_argument('--in-raw-dir', required=True, type=str, help='input directory with wavs')
    parser.add_argument('--out-xvec-dir', required=True, type=str, help='output directory with x-vectors')
    ...
    output_file = os.path.join(args.out_xvec_dir, os.path.basename(args.in_file_list))
    with torch.no_grad(), open(output_file, "w") as f:
        ...
        for fn in file_names:
            ...
            signal = np.fromfile(f'{os.path.join(args.in_raw_dir, fn)}.raw', dtype=np.int16)
            ...
            xvector = model.forward_backend(fea)

            # key = f'{fn}_{segnum:04}-{start:08}-{(start + seg_len):08}'
            if args.task == "cat":
                key = os.path.basename(os.path.dirname(fn))
            elif args.task == "nocat":
                key = os.path.basename(os.path.dirname(os.path.dirname(fn)))
            else:
                print()
                sys.exit(0)
            ...
            # output_xvec_file = os.path.join(args.out_xvec_dir, key+".xvec.gz")
            # os.makedirs(os.path.dirname(output_xvec_file), exist_ok=True)
            # np.savetxt(output_xvec_file, xvector)
            # print(args.out_xvec_dir, os.path.basename(args.in_file_list))
            # output_file = os.path.join(args.out_xvec_dir, os.path.basename(args.in_file_list))
            # with open(output_file, "w") as f:
            print(f"{key} {vec2str(xvector)} {fn}", file=f)
        ...
        #     key = f'{fn}-{(start + seg_jump):08}-{slen:08}'
        #     if np.isnan(xvector).any():
        #         logger.warning(f'NaN found, not processing: {key}{os.linesep}')
        #     # else:
        #     #     seg_start = round(
